...
from .models import Quiz, Question, Choice,Code,Candidate
...
def single_quiz(request, quiz_id,token):
    quiz = get_object_or_404(Quiz, pk=quiz_id)
    num_questions = len(quiz.question_set.all())


    # Code.objects.filter(token=token).exists()
    if(token=="admin"):
        if num_questions == 0:
            quiz.delete()
            all_quiz_list = Quiz.objects.all()
    ...
    if(Code.objects.filter(token=token).exists()):
        ...
        return render(request, "quiz/expired.html")


# specific question view
def single_question(request, quiz_id, question_id,name):
    quiz = get_object_or_404(Quiz, pk=quiz_id)
    current_question = quiz.question_set.get(question_num=question_id)
    gem=""
    # get correct answer and sent it with response 
    try:
        gem = current_question.choice_set.get(correct=True)
    except:
        pass
...
def vote(request, quiz_id, question_id,name):
    quiz = get_object_or_404(Quiz, pk=quiz_id)
    current_question = quiz.question_set.get(question_num=question_id)
    ...
    try:
        ...
        if question_id == (len(quiz.question_set.all())):
            ...
            return HttpResponseRedirect(reverse("quiz:results", args=(quiz.id,name)))
        else:
...
# quiz results page
def results(request, quiz_id,name):
    quiz = get_object_or_404(Quiz, pk=quiz_id)
    ...
    obj=Candidate.objects.get(name=name)
    obj.total_qn=total_questions
    obj.correct_qn=num_correct
    obj.wrong_qn=num_wrong
    obj.save()
...
def allquiz(request):
    ...
    return render(request, "quiz/allquiz.html", context)

def code_to_exam(request):
    quizid=""
    if request.method == "POST":
        
        name=request.POST["name"]
        token=request.POST["code"]
        print(request.POST["name"] ,request.POST["code"])
        Code.objects.filter(token=token).exists()
        if(Code.objects.filter(token=token).exists()):
            cc=Code.objects.get(token=token)
            quizid=cc.quizid
            ...
            return render(request, "quiz/expired.html")        # Code.objects.all()
...
def register_exam(request):
    ...
    if request.method == "POST":
        ...
        print(request.POST["name"] ,request.POST["mailid"])
        codeobj = Code(name=name, email=mailid,token=token,isexpired=False,quizid=quizid)
        codeobj.save()
        mymail.sendmail(mailid,"your token for Skill Ninja quiz",
        """
        your token
...
def admin_single_quiz(request, quiz_id):
    quiz = get_object_or_404(Quiz, pk=quiz_id)
    num_questions = len(quiz.question_set.all())
...
def candidate_update(request,quiz_id,question_id,token):
    cc=Code.objects.get(token=token)
    email=cc.email
    name=cc.name
